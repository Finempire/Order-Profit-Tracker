...
export async function DELETE(
  ...
  try {
    await prisma.overheadCostEntry.delete({ where: { id: entryId } });
    return successResponse(null, "Overhead entry deleted");
  } catch (err) {
    console.error(err);
    return errorResponse("Failed to delete overhead entry", 500);
  }
}
