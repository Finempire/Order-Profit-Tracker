...
export async function DELETE(
  ...
  try {
    await prisma.labourCostEntry.delete({ where: { id: entryId } });
    return successResponse(null, "Labour entry deleted");
  } catch (err) {
    console.error(err);
    return errorResponse("Failed to delete labour entry", 500);
  }
}
